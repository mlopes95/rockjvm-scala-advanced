...
object Givens extends App {
  ...
  object GivenAnonymousClassNaive {
    given descendingOrder_v2: Ordering[Int] = new Ordering[Int] {
  ...
  // mplicit conversions (abused in Scala 2)
  case class Person(name: String) {
    def greet(): String = s"Hi, my name is $name."
  }

  implicit def string2Person(string: String): Person = Person(string)
  val danielGreet = "Daniel".greet() // string2Person("Daniel").greet()
